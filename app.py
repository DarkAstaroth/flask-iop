...
def generate_token():
    try:
        ...
        if unit not in ["day", "week", "month"]:
            return jsonify({"error": "Unidad no válida. Use 'day', 'week' o 'month'"}), 400
        ...
        if response.status_code != 200:
            return jsonify({"error": "No se pudo obtener los datos del consumidor"}), 500

        # Extraer la clave (key) y el secreto (secret) del consumidor
        consumer_data = response.json()
        if not consumer_data.get("data"):
            return jsonify({"error": "No se encontraron credenciales JWT para el consumidor"}), 404
        ...
        if not consumer_key or not consumer_secret:
            return jsonify({"error": "El consumidor no tiene credenciales JWT válidas"}), 404
        ...
        if unit == "day":
            ...
            expiration_time = current_time + (value * 604800)  # 1 semana = 604800 segundos
        elif unit == "month":
